...
export default function ReportList() {
  ...
  useEffect(() => {
    ...
    }, (err) => console.error("Legacy reports listener failed:", err));
    ...
    }, (err) => console.error("Professional reports listener failed:", err));
    ...
    <div className="space-y-6">
      ...
      {reports.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* ... mapping remains mostly the same, but let's ensure it's robust ... */}
          {reports.map((report) => (
            <div key={report.id} className="bg-white p-6 border border-zinc-200 rounded-xl shadow-sm space-y-3 relative overflow-hidden flex flex-col">
              ...
              <div className="pt-3 border-t border-zinc-100 text-sm text-zinc-700 mt-auto">
                <p className="line-clamp-3 italic text-zinc-500">
                  ...
                  {report.isProfessional ? report.dutySummary : report.why}
                </p>
              </div>
